...
static int si1145_check_reg(SI1145_REG reg, uint8_t expected);
static int si1145_read_reg(SI1145_REG reg, uint8_t *data);
static int si1145_write_reg(SI1145_REG reg, uint8_t data);
static int si1145_write_check_reg(SI1145_REG reg, uint8_t data);
static int si1145_check_ram(SI1145_RAM ram, uint8_t expected);
static int si1145_read_ram(SI1145_RAM ram, uint8_t *data);
static int si1145_write_ram(SI1145_RAM ram, uint8_t data);
static int si1145_check_status(void);
static int si1145_send_cmd(SI1145_CMD cmd, uint8_t cmd_low_bits);
...
static int si1145_check_reg(SI1145_REG reg, uint8_t expected)
{
    ...
    if (si1145_read_reg(reg, &data) != HAL_OK)//IO_SPA_OK
    {
...
static int si1145_read_reg(SI1145_REG reg, uint8_t *data)
{
    ...
    return HAL_I2C_Master_Receive(&i2c1, reg, data, 1,0xff);//HAL_ERROR    = 0x01,HAL_OK       = 0x00,
}

static int si1145_write_reg(SI1145_REG reg, uint8_t data)
{
    if (HAL_I2C_Master_Transmit(&i2c1, reg, &data,1,0xff) != HAL_OK)
    {
        return SI1145_FAILURE;
    }
...
int si1145_init(const char *bus, uint8_t addr, uint8_t config_bitmap)
{
    uint8_t chlist = 0x0;
   // i2c1.dev_addr = addr;
//    if (HAL_I2C_Init(&i2c1))
//    {
//        return SI1145_FAILURE;
//    }
...
int si1145_measurement_auto(SI1145_MEASUREMENT_QUANTITY quantity)
{
    SI1145_CMD cmd;
    switch (quantity)
    {
...
int si1145_measurement_pause(SI1145_MEASUREMENT_QUANTITY quantity)
{
    SI1145_CMD cmd;
    switch (quantity)
    {
...
int si1145_measurement_force(SI1145_MEASUREMENT_QUANTITY quantity)
{
    SI1145_CMD cmd;
    switch (quantity)
    {
...
int si1145_get_vis_data(uint16_t *vis_data)
{
    uint8_t data[2];
...
int si1145_get_ir_data(uint16_t *ir_data)
{
    uint8_t data[2];
...
int si1145_get_ps_data(uint16_t *ps1_data, uint16_t *ps2_data, uint16_t *ps3_data)
{
    uint8_t data[6];
...
int si1145_get_uv_data(uint16_t *uv_data)
{
    uint8_t data[2];
